...
export default function GruposDeInvestigacionAreaPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { setItems(await apiFetch('/api/GruposDeInvestigacion/area')); }
  ...
  async function handleGenerarAnexo() {
    ...
    try {
      ...
      const now = new Date();
      a.download = `anexo-grupos_${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}.xlsx`;
      a.href = url;
      a.click();
  ...
  return (
    <>
      <h2 className="mb-4">Grupos de Investigación del Área</h2>
        ...
        <CardBody className="p-0">
          <FilterableDataTable
            ...
            columns={[
              ...
              { key: 'usuariosIds', label: 'Miembros', render: v => v?.length ?? 0 },
            ]}
            data={items}
            keyExtractor={i => i.id}
            emptyMessage="No hay grupos de investigación en el área."
            detailConfig
          />
        </CardBody>
      </Card>
